      ...
      <div className="mx-auto max-w-6xl px-4 space-y-8">
        ...
        <div className="bg-white rounded-md shadow-sm">
          <nav className="flex items-center gap-6 px-4 py-3">
            ...
            <button onClick={() => { setActiveTab('issues'); setAccountSearch(''); }} className={`text-sm font-medium ${activeTab === 'issues' ? 'text-blue-600 border-b-2 border-blue-600 pb-2' : 'text-gray-600'}`}>Issues Review</button>
          </nav>
        </div>
                                  ...
                                  <td className="whitespace-nowrap px-4 py-4 text-right text-sm">
                                    <div className="inline-flex items-center gap-1">
                                      ...
                                      <button
                                        ...
                                        onClick={(event) => {
                                          event.stopPropagation();
                                          setAccountSearch(String(it.borrowerNumber ?? ''));
                      ...
                      <div className="absolute right-0 mt-2 w-72 rounded-md border bg-white shadow z-20 p-2">
                        <div className="px-3 py-2">
                          ...
                          <div className="flex gap-2">
                            <button disabled={exporting} onClick={async () => { setShowExportMenu(false); setExporting(true); try { const blob = await accountService.exportAccounts({ format: 'csv', limit: 50000, search: accountSearch || undefined, status: accountStatus || undefined, batchNumber: accountBatch || undefined, startDate: accountStartDate || undefined, endDate: accountEndDate || undefined, institutionId: selectedInstitutionId || undefined }); const url = window.URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `accounts-${new Date().toISOString().split('T')[0]}.csv`; document.body.appendChild(a); a.click(); setTimeout(() => { a.remove(); window.URL.revokeObjectURL(url); }, 100); } catch (err) { toast.error(getApiErrorMessage(err, 'Failed to export CSV')); } finally { setExporting(false); } }} className="inline-flex items-center gap-2 rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-semibold"><FileText className="w-4 h-4" /> CSV</button>
                            <button disabled={exporting} onClick={async () => { setShowExportMenu(false); setExporting(true); try { const blob = await accountService.exportAccounts({ format: 'xlsx', limit: 50000, search: accountSearch || undefined, status: accountStatus || undefined, batchNumber: accountBatch || undefined, startDate: accountStartDate || undefined, endDate: accountEndDate || undefined, institutionId: selectedInstitutionId || undefined }); const url = window.URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `accounts-${new Date().toISOString().split('T')[0]}.xlsx`; document.body.appendChild(a); a.click(); setTimeout(() => { a.remove(); window.URL.revokeObjectURL(url); }, 100); } catch (err) { toast.error(getApiErrorMessage(err, 'Failed to export XLSX')); } finally { setExporting(false); } }} className="inline-flex items-center gap-2 rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-semibold"><FileSpreadsheet className="w-4 h-4" /> XLSX</button>
                          </div>
                        </div>
                      </div>
                    )}
