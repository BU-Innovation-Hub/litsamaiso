...
const AccountsPage: React.FC = () => {
  ...
  const [studentImport, setStudentImport] = useState<StudentImportState | null>(
    null,
  );
  const [financialImport, setFinancialImport] =
    useState<FinancialClearanceImportState | null>(null);
  ...
  const [reviewingIssue, setReviewingIssue] = useState<{
    id: string;
  ...
  const canViewReports = ["AppAdmin", "InstitutionAdmin", "Finance"].includes(
    role,
  );
  ...
  const loadReports = useCallback(async () => {
    ...
    try {
      setReports(
        await accountService.getReports({
          institutionId: selectedInstitutionId || undefined,
          batchNumber: accountBatch || undefined,
        }),
      );
  ...
  useEffect(() => {
    const loadFinanceIssues = async () => {
      ...
      setIssuesLoading(true);
      try {
        const list = await adminIssueService.listIssues({
          search: accountSearch || undefined,
        });
        setIssueList(list || []);
        setIssuePage(1);
    ...
    try {
        ...
        uploadType === "accounts"
          ? await accountService.uploadAccounts(file, (progress) => {
              setFinancialImport({
                ...
                message:
                  progress.message || "Importing financial clearance records",
          ...
          : uploadType === "paid"
            ? await accountService.uploadPaidStudents(file, (progress) => {
                setPaidImport({
                  fileName: file.name,
                  status:
                    progress.type === "completed" ? "completed" : "running",
                  percent: Math.max(0, Math.min(100, progress.percent || 0)),
      ...
      if (uploadType === "students") {
        setStudentImport((current) => ({
          ...
          status: "completed",
          processed:
            response.result?.total || current?.processed || current?.total || 0,
          total:
            response.result?.total || current?.total || current?.processed || 0,
    ...
    try {
      await Promise.all(
        selectedAccountIds.map((id) =>
          accountService.updateAccount(id, { status: "paid" } as any),
        ),
      );
      ...
      toast.error(
        getApiErrorMessage(error, "Failed to mark selected accounts as paid"),
      );
    }
  };
  ...
  const hasChanged = (original: unknown, proposed: unknown) => {
    ...
    return Boolean(
      originalValue && proposedValue && originalValue !== proposedValue,
    );
  };

  const getIssueProofImages = (issue: any) => {
    const urls = Array.isArray(issue.proofUrls)
      ? issue.proofUrls.filter(Boolean)
      : [];
    if (
      issue.documentBase64 &&
      String(issue.documentMimeType || "").startsWith("image/")
    ) {
      return [
        ...urls,
        `data:${issue.documentMimeType};base64,${issue.documentBase64}`,
      ];
    }
    return urls;
  };
  ...
  const totalIssuePages = Math.max(
    1,
    Math.ceil(financeIssueRows.length / issuePageSize),
  );
  ...
  const issueStart =
    financeIssueRows.length === 0
      ? 0
      : (currentIssuePage - 1) * issuePageSize + 1;
  const issueEnd = Math.min(
    currentIssuePage * issuePageSize,
    financeIssueRows.length,
  );
  const selectedIssueStatus = String(
    selectedIssue?.status || "submitted",
  ).toLowerCase();
  const selectedIssueIsPending =
    selectedIssueStatus === "submitted" || selectedIssueStatus === "reported";
  const currentReviewingIssue = reviewingIssue;
  const selectedIssueAction =
    selectedIssue &&
    currentReviewingIssue &&
    currentReviewingIssue.id === selectedIssue._id
      ? currentReviewingIssue.action
      : null;
  const selectedIssueCurrentBankName =
    selectedIssue?.recordedBankName || selectedIssue?.account?.bankName;
  const selectedIssueCurrentAccountNumber =
    selectedIssue?.recordedAccountNumber ||
    selectedIssue?.account?.accountNumber;
  const selectedIssueProposedBankName =
    selectedIssue?.correctedBankName || selectedIssue?.bankName;
  const selectedIssueProposedAccountNumber =
    selectedIssue?.correctedAccountNumber || selectedIssue?.accountNumber;
  const selectedIssueBankChanged = hasChanged(
    selectedIssueCurrentBankName,
    selectedIssueProposedBankName,
  );
  const selectedIssueAccountChanged = hasChanged(
    selectedIssueCurrentAccountNumber,
    selectedIssueProposedAccountNumber,
  );
  const selectedIssueProofImages = selectedIssue
    ? getIssueProofImages(selectedIssue)
    : [];
  const selectedIssueReasons =
    selectedIssue && Array.isArray(selectedIssue.reasons)
        ...
        <div className="bg-white rounded-md shadow-sm">
          <nav className="flex items-center gap-6 px-4 py-3">
            <button
              ...
              className={`text-sm font-medium ${activeTab === "records" ? "text-blue-600 border-b-2 border-blue-600 pb-2" : "text-gray-600"}`}
            >
              Account Details
            </button>
            <button
              ...
              className={`text-sm font-medium ${activeTab === "issues" ? "text-blue-600 border-b-2 border-blue-600 pb-2" : "text-gray-600"}`}
            >
              Issues Review
            </button>
          </nav>
          ...
          <div className="rounded-lg bg-white p-6 shadow">
            ...
            {reports?.scope && (
              <p className="mt-2 text-sm text-muted-foreground">
                Viewing{" "}
                {reports.scope.allInstitutions
                  ? "all institutions"
                  : reports.scope.institutionName || "selected institution"}
                .
              </p>
            )}
          </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Student-submitted issues and finance resolutions.
                </p>
              </div>
            </div>
            ...
            <div className="mt-4">
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <button
                      onClick={async () => {
                        ...
                        try {
                          ...
                          toast.error(
                            getApiErrorMessage(err, "Failed to refresh issues"),
                          );
                        } finally {
                          setIssuesLoading(false);
                        }
                    ...
                    <button
                      onClick={async () => {
                        try {
                          const res =
                            await issueService.deleteIssuesForStudent();
                          toast.success(res.message || "Deleted issues");
                          setIssueList([]);
                        } catch (err) {
                          toast.error(
                            getApiErrorMessage(err, "Failed to delete issues"),
                          );
                        }
                      }}
                          ...
                          {issueList.map((it) => (
                            <tr key={(it as any)._id}>
                              <td className="px-6 py-4 text-sm font-medium text-gray-900">
                                {(it as any).borrowerNumber}
                              </td>
                              <td className="px-6 py-4 text-sm text-gray-500">
                                {(it as any).bankName}
                              </td>
                              <td className="px-6 py-4 text-sm text-gray-500">
                                {(it as any).accountNumber}
                              </td>
                              <td className="px-6 py-4 text-sm text-gray-500">
                                {(it as any).status}
                              </td>
              ...
              {role === "Finance" && (
                <div>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          toast.error(
                            getApiErrorMessage(err, "Failed to load issues"),
                          );
                          setIssueList([]);
                        } finally {
                          setIssuesLoading(false);
                  ...
                  {issuesLoading ? (
                    ...
                    <div className="rounded-md border border-gray-200 bg-gray-50 px-4 py-6 text-gray-600">
                      No issues found
                    </div>
                  ) : (
                    <div className="overflow-hidden rounded-lg border border-slate-200">
                          ...
                          <tbody className="divide-y divide-slate-100 bg-white">
                            {paginatedIssueRows.map((it: any) => {
                              ...
                              const reasons = Array.isArray(it.reasons)
                                ? it.reasons
                                : [];
                              const proposedBankName =
                                it.correctedBankName || it.bankName;
                              const proposedAccountNumber =
                                it.correctedAccountNumber || it.accountNumber;
                              ...
                              return (
                                ...
                                >
                                  <td className="whitespace-nowrap px-4 py-4 text-sm font-semibold text-slate-950">
                                    {displayValue(it.borrowerNumber)}
                                  </td>
                                  <td className="whitespace-nowrap px-4 py-4 text-sm text-slate-600">
                                    {(it.student && it.student.studentId) ||
                                      it.studentId ||
                                      "-"}
                                  </td>
                                  ...
                                  <td className="px-4 py-4 text-sm text-slate-700">
                                    <p className="font-medium text-slate-950">
                                      {displayComparableValue(proposedBankName)}
                                    </p>
                                    <p className="text-slate-500">
                                      {displayValue(proposedAccountNumber)}
                                    </p>
                                  </td>
                                  <td className="whitespace-nowrap px-4 py-4 text-sm text-slate-600">
                                    <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-700">
                                      ...
                                      {proofImages.length ||
                                        (it.documentFileName ? 1 : 0)}
                                    </span>
                                  </td>
                                  <td className="whitespace-nowrap px-4 py-4 text-sm">
                                    <span
                                      className={`rounded-full px-2.5 py-1 text-xs font-semibold ${getIssueStatusBadgeClass(issueStatus)}`}
                                    >
                                      {issueStatus}
                                    </span>
                                  </td>
                                    ...
                                    <div className="inline-flex items-center gap-1">
                                      ...
                                      <button
                                        ...
                                        onClick={(event) => {
                                          event.stopPropagation();
                                          setAccountSearch(
                                            String(it.borrowerNumber ?? ""),
                                          );
                                          setActiveTab("records");
                                        }}
                                        className="rounded-md p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-700"
                                        title="Find in Account Details"
                                      >
                      ...
                      <div className="flex flex-col gap-3 border-t border-slate-200 bg-slate-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                        <p className="text-sm text-slate-600">
                          Showing {issueStart}-{issueEnd} of{" "}
                          {financeIssueRows.length} issues
                        </p>
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            onClick={() =>
                              setIssuePage((page) => Math.max(1, page - 1))
                            }
                          ...
                          <button
                            ...
                            onClick={() =>
                              setIssuePage((page) =>
                                Math.min(totalIssuePages, page + 1),
                              )
                            }
                            className="inline-flex items-center gap-1 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 disabled:cursor-not-allowed disabled:opacity-50"
                          >
              ...
              {role !== "Student" && role !== "Finance" && (
                <p className="text-gray-600">
                  Issue lists are only available to selected finance teams.
                  Finance users can resolve issues by Student ID.
                </p>
              )}
            </div>
          </div>
            ...
            <aside className="absolute right-0 top-0 flex h-full w-full max-w-4xl flex-col bg-white shadow-2xl">
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="text-lg font-semibold text-slate-950">
                      {displayValue(selectedIssue.borrowerNumber)}
                    </h3>
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-semibold ${getIssueStatusBadgeClass(selectedIssueStatus)}`}
                    >
                      {selectedIssueStatus}
                    </span>
                  </div>
                  <p className="mt-1 text-sm text-slate-600">
                    ...
                    {(selectedIssue.student &&
                      selectedIssue.student.studentId) ||
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5">
                <div className="mb-4 flex flex-wrap gap-2">
                  {selectedIssueReasons.length > 0 ? (
                    selectedIssueReasons.map((reason: string) => (
                      <span
                        key={reason}
                  ...
                  <div className="grid gap-0 lg:grid-cols-[1fr_auto_1fr]">
                    <section className="px-5 py-5">
                      ...
                      <dl className="space-y-3">
                        ...
                        <div>
                          <dt className="text-xs uppercase text-slate-500">
                            Account number in system
                          </dt>
                          <dd className="mt-1 text-sm font-medium text-slate-950">
                            {displayValue(selectedIssueCurrentAccountNumber)}
                          </dd>
                        </div>
                    ...
                    <section className="border-t border-slate-200 px-5 py-5 lg:border-t-0">
                      ...
                      <dl className="space-y-3">
                        ...
                        <div>
                          ...
                          <dd className="mt-1 text-sm font-medium text-slate-950">
                            {displayValue(selectedIssueProposedAccountNumber)}
                          </dd>
                        </div>
                        {selectedIssue.notes && (
                          <div>
                  ...
                  <div className="border-t border-slate-200 px-5 py-5">
                    ...
                    {selectedIssueProofImages.length > 0 ? (
                      <div className="flex flex-wrap gap-2">
                        {selectedIssueProofImages.map(
                          (url: string, idx: number) => (
                            <button
                              key={`${url}-${idx}`}
                              type="button"
                              onClick={() => {
                                setLightboxImages(selectedIssueProofImages);
                            ...
                            >
                              <img
                                src={url}
                                alt={`proof-${idx}`}
                                className="h-24 w-32 rounded object-cover transition group-hover:opacity-90"
                              />
                            </button>
                          ),
                        )}
                  ...
                  <div className="flex flex-wrap justify-end gap-2">
                    <button
                      ...
                      onClick={async () => {
                        if (
                          !confirm(
                            "Approve this issue and update the student's account details?",
                          )
                        )
                          return;
                        setReviewingIssue({
                        ...
                        try {
                          await adminIssueService.approveIssue(
                            selectedIssue._id,
                          );
                          ...
                          toast.error(
                            getApiErrorMessage(err, "Failed to approve issue"),
                          );
                        } finally {
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          await adminIssueService.rejectIssue(
                            selectedIssue._id,
                            reason || undefined,
                          );
                          ...
                          toast.error(
                            getApiErrorMessage(err, "Failed to reject issue"),
                          );
                        } finally {
                          setReviewingIssue(null);
                        }
            ...
            <div className="relative z-10 w-full max-w-2xl rounded-lg bg-white p-6 shadow-lg">
              ...
              <form onSubmit={handleEditSubmit} className="mt-4 space-y-4">
                <div>
                  ...
                  <input
                    value={String(editForm.fullnames || "")}
                    onChange={(e) =>
                      setEditForm((p) => ({ ...p, fullnames: e.target.value }))
                    }
                ...
                <div className="grid grid-cols-2 gap-4">
                  <input
                    value={String(editForm.borrowerNumber || "")}
                    onChange={(e) =>
                      setEditForm((p) => ({
                        ...p,
                        borrowerNumber: e.target.value,
                      }))
                    }
                    className="w-full rounded-md border border-gray-300 px-3 py-2"
                  />
                  <input
                    value={String(editForm.courseOfStudy || "")}
                    onChange={(e) =>
                      setEditForm((p) => ({
                        ...p,
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  />
                  <input
                    value={String(editForm.accountNumber || "")}
                    onChange={(e) =>
                      setEditForm((p) => ({
                        ...p,
                ...
                <div>
                  ...
                  <select
                    value={String(editForm.status || "pending")}
                    onChange={(e) =>
                      setEditForm((p) => ({
                        ...p,
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <button
                    type="submit"
                    className="rounded-md bg-active px-4 py-2 text-white"
                  >
                    Save
                  </button>
                </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="bg-white rounded-lg border border-border p-6 shadow-sm">
              ...
              <p className="text-muted-foreground mb-5">
                Submit your borrower's number and banking details so your
                institution can verify them.
              </p>
              <Link
                to="/accounts/confirm"
            ...
            >
              ...
              <div>
                ...
                <p className="text-muted-foreground">
                  Use this when Finance needs your corrected bank details and
                  proof document.
                </p>
              </div>
              <input
                ...
                onChange={(event) =>
                  setCorrection((prev) => ({
                    ...prev,
                    correctedBankName: event.target.value,
                  }))
                }
              ...
              <input
                ...
                onChange={(event) =>
                  setCorrection((prev) => ({
                    ...prev,
                    correctedAccountNumber: event.target.value,
                  }))
                }
              ...
              />
              <button
                className="w-full rounded-md bg-button py-2 font-semibold text-white"
                type="submit"
              >
                Submit correction
              </button>
                  ...
                  {role !== "InstitutionAdmin" && (
                    <div>
                      ...
                      >
                        ...
                        {financialImport?.status === "running"
                          ? "Importing..."
                          : "Import Financial Clearance List"}
                      </button>
                      <input
                        ref={accountsFileRef}
                        type="file"
                        accept=".xlsx,.xls,.csv"
                        onChange={(e) => handleUpload(e, "accounts")}
                        className="hidden"
                      />
                    </div>
                  )}
                  {role !== "InstitutionAdmin" && (
                    <div>
                      ...
                      <input
                        ...
                        type="file"
                        accept=".xlsx,.xls,.csv"
                        onChange={(e) => handleUpload(e, "paid")}
                        className="hidden"
                      />
                    </div>
                  )}
                            ...
                            <button
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  const a = document.createElement("a");
                                  a.href = url;
                                  a.download = `accounts-${new Date().toISOString().split("T")[0]}.xlsx`;
                                  document.body.appendChild(a);
                                  a.click();
                                  setTimeout(() => {
                                    a.remove();
            ...
            {financialImport && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 py-6 backdrop-blur-sm">
                <div className="w-full max-w-lg rounded-2xl border border-blue-100 bg-white p-5 shadow-2xl">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-sm font-semibold text-gray-900">
                          Financial clearance import
                        </h3>
                      </div>
                      <p className="mt-1 text-sm text-gray-600">
                        {financialImport.message ||
                          "Importing financial clearance list"}
                        : {financialImport.fileName}
                      </p>
                    </div>
                    <span className="text-sm font-semibold text-blue-800">
                      {financialImport.percent}%
                    </span>
                  </div>
                  <div className="mt-4 h-2.5 overflow-hidden rounded-full bg-gray-100">
                    <div
                      className={`h-full rounded-full transition-all duration-300 ${
                        financialImport.status === "error"
                          ? "bg-red-600"
                          : "bg-blue-600"
                      }`}
                  ...
                  {financialImport.total != null &&
                    financialImport.total > 0 && (
                      <div className="mt-4 grid gap-2 text-xs text-gray-700 sm:grid-cols-4">
                        <span>
                          Processed {financialImport.processed ?? 0}/
                          {financialImport.total}
                        </span>
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 py-6 backdrop-blur-sm">
                <div className="w-full max-w-lg rounded-2xl border border-blue-100 bg-white p-5 shadow-2xl">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-sm font-semibold text-gray-900">
                          Paid accounts import
                        </h3>
                      </div>
                      <p className="mt-1 text-sm text-gray-600">
                        {paidImport.message || "Importing paid records"}:{" "}
                        {paidImport.fileName}
                      </p>
                    </div>
                    <span className="text-sm font-semibold text-blue-800">
                  ...
                  {paidImport.total != null && paidImport.total > 0 && (
                    <div className="mt-4 grid gap-2 text-xs text-gray-700 sm:grid-cols-4">
                      <span>
                        Processed {paidImport.processed ?? 0}/{paidImport.total}
                      </span>
                ...
                <div className="w-full max-w-lg rounded-2xl border border-blue-100 bg-white p-5 shadow-2xl">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-600">
                        {studentImport.message || "Importing student list"}:{" "}
                        {studentImport.fileName}
                      </p>
                    </div>
                    <span className="text-sm font-semibold text-blue-800">
                      {studentImport.percent}%
                    </span>
                  </div>
                  <div className="mt-4 h-2.5 overflow-hidden rounded-full bg-gray-100">
                    <div
                      className={`h-full rounded-full transition-all duration-300 ${
                        studentImport.status === "error"
                          ? "bg-red-600"
                          : "bg-blue-600"
                      }`}
                  ...
                  <div className="mt-4 grid gap-2 text-xs text-gray-700 sm:grid-cols-4">
                    <span>
                      Processed {studentImport.processed}
                      {studentImport.total ? `/${studentImport.total}` : ""}
                    </span>
              ...
              <div className="flex flex-col gap-4 rounded-lg border border-gray-200 bg-gray-50 p-4 lg:flex-row lg:items-center">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  <div className="flex flex-col lg:flex-row lg:items-center gap-2">
                    <label className="text-xs text-gray-500 lg:mr-2">
                      From
                    </label>
                    <input
                      type="date"
                      ...
                      onChange={(event) =>
                        setAccountStartDate(event.target.value)
                      }
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center gap-2">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setAccountEndDate(event.target.value)
                      }
                      className="rounded-md border border-gray-300 bg-white px-3 py-2 text-sm"
                    />
                  </div>
              ...
              <div className="flex items-center justify-between px-6 py-3 border-b bg-gray-50">
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-md bg-active px-3 py-1 text-white text-sm font-semibold"
                  >
                    Mark as paid
                  </button>
                  <button
                    onClick={exportSelectedAsJson}
                    className="inline-flex items-center gap-2 rounded-md border border-gray-300 bg-white px-3 py-1 text-sm font-semibold"
                  >
                    Export JSON
                  </button>
                </div>
                    ...
                    accounts.map((account) => (
                      <tr key={account._id}>
                        ...
                        <td className="whitespace-nowrap px-6 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            {["Finance", "AppAdmin"].includes(role) ? (
                              ...
                              <span
                                className={`rounded-full px-2 py-1 text-xs font-semibold ${getStatusColor(account.status)}`}
                              >
                        ...
                        <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                          ...
                          <p>
                            Paid:{" "}
                            {account.paidAt || account.paidDate
                              ? new Date(
                                  account.paidAt || account.paidDate || "",
                                ).toLocaleDateString()
                              : "-"}
                          ...
                          <div className="inline-flex items-center gap-2">
                            ...
                            <button
                              onClick={() => {
                                ...
                                a.download = `account_${account.borrowerNumber || account._id}.json`;
                                document.body.appendChild(a);
                                a.click();
                                a.remove();
                              }}
  ...
  <div className="rounded-lg bg-white p-6 shadow">
    ...
    {typeof percent === "number" && (
      <div className="mt-3 h-2 w-full rounded-full bg-gray-100">
        <div
          className="h-2 rounded-full bg-active"
