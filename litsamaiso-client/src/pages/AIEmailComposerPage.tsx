...
const normalizeRoleName = (roleName: string) => {
  ...
  if (normalized === "appadmin" || normalized === "appadmins")
    return "AppAdmin";
...
const AIEmailComposerPage: React.FC = () => {
  ...
  const [financialStatus, setFinancialStatus] =
    useState<FinancialStatus>("pending");
  ...
  const loadRecipientCount = useCallback(async () => {
    ...
    try {
      setRecipientCount(
        await aiEmailComposerService.countRecipients(selection),
      );
    } catch (error: unknown) {
      setRecipientCount(0);
  ...
  useEffect(() => {
    const loadRoles = async () => {
      try {
        ...
        const studentRole = loadedRoles.find((role) =>
          isStudentRole(role.name),
        );
        if (studentRole) {
  ...
  useEffect(() => {
    const loadBatches = async () => {
      ...
      try {
        ...
        toast.error(
          getApiErrorMessage(error, "Failed to load available batches"),
        );
      } finally {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">
              Compose branded administrative emails with Gemini and send them
              through Litsamaiso.
            </p>
          </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
          <div className="space-y-6">
            ...
            <section className="rounded-lg bg-white px-6 pt-6 pb-8 shadow">
              <div className="flex items-center gap-3">
                <StepBadge number={2} />
          ...
          <div className="space-y-6">
            <section className="rounded-lg bg-white px-6 pt-6 pb-1 shadow">
              ...
              <div className="relative mt-5 space-y-4">
                ...
                <section className="absolute w-full bottom-0 rounded-lg bg-white p-3 shadow">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex gap-3">
                      <Mail className="mt-1 h-5 w-5 text-active-clr" />
                      <div>
                        <h2 className="text-lg font-semibold text-gray-900">
                          Broadcast Email
                        </h2>
                        <p className="text-sm text-gray-600">
                          {countLoading
                            ? "Refreshing recipient count..."
                            : `${recipientCount} matching recipients`}
                        </p>
                      </div>
                    </div>
                    <button
                    ...
                    <div className="mt-5 rounded-lg border border-gray-200 bg-gray-50 p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            Total {job.totalRecipients} - Sent{" "}
                            {job.successfulSends} - Failed {job.failedSends}
                          </p>
                        </div>
                      </div>
                      {job.lastError && (
