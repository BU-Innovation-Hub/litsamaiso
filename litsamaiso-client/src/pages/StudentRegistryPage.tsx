      ...
      <div className="w-full max-w-lg rounded-2xl border border-blue-100 bg-white p-5 shadow-2xl">
        ...
        <div className="mt-4 grid gap-2 text-xs text-gray-700 sm:grid-cols-4">
          <span>Processed {state.processed ?? 0}{state.total ? `/${state.total}` : ''}</span>
           ...
           <span>Updated {state.updated ?? 0}</span>
...
type ExceptionForm = { fullnames: string; name: string; surname: string; email: string; studentId: string; nationalId: string; borrowerNumber: string; courseOfStudy: string; bankName: string; accountNumber: string; studentStatus: boolean };

const ExceptionModal: React.FC<{ exception: RegistryException; onClose: () => void; onChanged: () => void }> = ({ exception, onClose, onChanged }) => {
  const [form, setForm] = useState<ExceptionForm>({ fullnames: exception.fullnames || '', name: exception.name || '', surname: exception.surname || '', email: exception.email || '', studentId: exception.studentId || '', nationalId: exception.nationalId || '', borrowerNumber: exception.borrowerNumber || '', courseOfStudy: exception.courseOfStudy || '', bankName: exception.bankName || '', accountNumber: exception.accountNumber || '', studentStatus: exception.studentStatus ?? true });
  const [busy, setBusy] = useState(false);
  ...
  const fields: Array<[keyof ExceptionForm, string]> = exception.importKind === 'students'
    ? [['name', 'Name'], ['surname', 'Surname'], ['email', 'Email'], ['studentId', 'Student ID'], ['nationalId', "Student's National ID"], ['borrowerNumber', 'Borrower Number']]
    : [['fullnames', 'Full Names'], ['nationalId', "Student's National ID"], ['borrowerNumber', 'Borrower Number'], ['courseOfStudy', 'Course of Study'], ['bankName', 'Bank Name'], ['accountNumber', 'Account Number']];
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <button type="button" aria-label="Close" onClick={onClose} className="absolute inset-0 bg-slate-950/40" />
      <form onSubmit={submit} className="relative z-10 max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-white p-6 shadow-2xl">
        ...
        {reconciliation && (
         ...
         <div className="mt-5 grid gap-4 sm:grid-cols-2">{fields.map(([key, label]) => <label key={key} className="text-sm font-medium text-slate-700">{label}<input value={String(form[key])} onChange={(event) => set(key, event.target.value)} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 font-normal outline-none focus:border-active-clr" /></label>)}</div>
         {exception.importKind === 'students' && <label className="mt-4 flex items-center gap-2 text-sm font-medium text-slate-700"><input type="checkbox" checked={form.studentStatus} onChange={(event) => set('studentStatus', event.target.checked)} /> Active student</label>}
        {confirmingDelete ? (
...
const StudentRegistryPage: React.FC = () => {
  ...
  const runUpload = async (kind: 'students' | 'financial', file: File) => {
     setUploadState({ kind, fileName: file.name, status: 'running', percent: 0, message: 'Preparing spreadsheet', processed: 0, total: 0, inserted: 0, updated: 0, skipped: 0, errors: 0 });
    ...
    try {
      const imported = kind === 'students' ? await registryService.uploadStudents(file, progress) : await registryService.uploadFinancial(file, progress);
       await registryService.applyImport(imported._id, progress);
     ...
     } catch (error) { setUploadState((current) => current ? { ...current, status: 'error', message: getApiErrorMessage(error, 'Import failed'), errors: Math.max(1, current.errors || 0) } : current); toast.error(getApiErrorMessage(error, 'Import failed')); }
  };
